...
import { IconCheck as CheckIcon, IconCircleCheck as CircleCheckIcon, IconCircleX as CircleXIcon, IconX as XIcon } from "@tabler/icons-react"
import {
  type ReactNode,
  useCallback,
  useEffect,
...
export function DeviceAuthorization({ className }: DeviceAuthorizationProps) {
  ...
  // The ref dedupes submissions synchronously; this copy is what renders.
...
function DeviceApproval({
  ...
  return (
    <Card className={className}>
      <CardHeader>
        <CardTitle>{localization.approveDevice}</CardTitle>
        <CardDescription>
      ...
      <CardContent>
        <div className="flex flex-col gap-3 rounded-lg border border-stroke-soft-200 bg-bg-weak-50 p-3">
          <div className="flex flex-col gap-1">
            <p className="text-paragraph-xs text-text-sub-600">
              {localization.deviceCode}
            </p>
            <p className="font-mono text-label-sm tracking-wider text-text-strong-950">
              {userCode}
            </p>
          </div>
          ...
          <div className="flex flex-col gap-1">
            ...
            <p className="text-label-sm text-text-strong-950">{user.name || user.email}</p>
            {user.name ? (
    ...
    <Card className={className}>
      <CardHeader className="items-center text-center">
        <Icon
          ...
          className={cn(
            "mb-1 size-10",
            approved ? "text-primary-base" : "text-error-base"
          )}
        />
        <CardTitle>
